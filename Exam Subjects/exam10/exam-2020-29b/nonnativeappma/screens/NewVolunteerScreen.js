...
const NewVolunteerScreen = props => {
  ...
  const isAvailable = (data) => {
    ...
    const request = fetch('http://10.0.2.2:2902/request');

    return Promise
      .race([timeout, request])
      .then(response => {
        alert('Action is supported in the server :)')
        fetch("http://10.0.2.2:2902/request", {
          method: "POST",
          cache: 'no-cache',
          headers: {
  ...
  const savePlaceHandler = async () => {
    const data = {
      name: nameValue, status: statusValue, student: driver, eCost: statusPassengers, cost: statusCapacity
    }
    isAvailable(data)
    ...
    <ScrollView>
      <View style={styles.form}>
        ...
        <TextInput
          ...
          value={statusValue}
        />
        <Text style={styles.label}>ECost</Text>
        <TextInput
