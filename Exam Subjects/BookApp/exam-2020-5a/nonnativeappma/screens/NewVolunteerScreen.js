...
const NewVolunteerScreen = props => {
  const student = useSelector(state => state.volunteers.name);
  ...
  const isAvailable = (data) => {
    ...
    const request = fetch('http://10.0.2.2:2501/book');

    return Promise
      .race([timeout, request])
      .then(response => {
        alert('Action is supported in the server :)')
        fetch("http://10.0.2.2:2501/book", {
          method: "POST",
          cache: 'no-cache',
          headers: {
  ...
  const savePlaceHandler = async () => {
    ...
    const data = {
      title: nameValue, status: statusValue, student:student, pages: statusPassengers, usedCount: statusCapacity
    }
    isAvailable(data)
