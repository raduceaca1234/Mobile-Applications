...
const VolunteersListScreen = props => {
    ...
    const isAvailable = (operationsQueue) => {
        ...
        const request = fetch('http://10.0.2.2:2501/available');
            ...
            <VolunteerItem
                ...
                onSelect={() => {
                    props.navigation.navigate('Borrow', {
                        volunteerName: itemData.item.title,
                        volunteerId: itemData.item.id,
                        volunteerStatus: itemData.item.status,
                        volunteerPassengers: itemData.item.pages,
                        volunteerDriver: itemData.item.student,
                        volunteerCapacity: itemData.item.usedCount
                    });
                }}
            />
        )}
