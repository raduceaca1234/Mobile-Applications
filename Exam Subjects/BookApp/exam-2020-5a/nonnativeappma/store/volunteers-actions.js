...
import { fetchAll, bookGame, fetchVolunteersLocation, fetchPaints, fetchVolunteersMe, fetchVolunteers, deleteVolunteerForm, createVolunteer, updateVolunteerForm } from '../constants/api';
...
export const bookGames = (id, student) => {
    return async dispatch => {
        try {
            const dbResult = await bookGame({id, student});
...
export const loadAll = () => {
    return async dispatch => {
        try {
            const dbResult = await fetchAll();
            dispatch({ type: SET_ALL, volunteers: dbResult });
        } catch (err) {
            throw err;
        }
    };
