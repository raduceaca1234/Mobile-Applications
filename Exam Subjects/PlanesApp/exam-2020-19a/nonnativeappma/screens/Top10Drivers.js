...
const Top10Drivers = props => {
    ...
    const driversNames = new Set(volunteers.map(item => item.owner));
    let data = [];
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        const nr = volunteers.filter(
            volunteer => volunteer.owner === array[i]
        )       
                ...
                <VolunteerItem
                    ...
                    onSelect={() => {
                        props.navigation.navigate('VolunteerDetail', {
                            volunteerName: itemData.item.driver,
                            volunteerPassengers: itemData.item.nr,
                        });
                    }}
                />
            )}
