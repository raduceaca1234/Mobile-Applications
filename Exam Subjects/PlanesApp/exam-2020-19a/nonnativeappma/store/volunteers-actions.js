...
import { fetchVolunteersColor, fetchPaints, fetchVolunteersMe, fetchVolunteers, deleteVolunteerForm, createVolunteer, updateVolunteerForm } from '../constants/api';
...
export const SET_CABS_WITH_PAINT = 'SET_CABS_WITH_PAINT';
...
export const loadVolunteersFromServerColors = (color) => {
    return async dispatch => {
        try {
            const dbResult = await fetchVolunteersColor({color});
            dispatch({ type: SET_CABS_WITH_PAINT, volunteers: dbResult });
        } catch (err) {
            throw err;
        }
    };
