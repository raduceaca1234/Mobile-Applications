...
const AllVechicles = props => {
    ...
    const data = volunteers.sort((a, b) => (a.type > b.type) ? 1 : ((b.type > a.type) ? -1 : 0));
    const data2 = data.sort((a, b) => (a.students < b.students) ? 1 : ((b.students < a.students) ? -1 : 0));

    const isAvailable = (operationsQueue) => {
        const timeout = new Promise((resolve, reject) => {
            setTimeout(reject, 300, 'Request timed out');
        });
        const request = fetch('http://10.0.2.2:2018/group/' + driver);
