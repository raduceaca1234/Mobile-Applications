...
const NewVolunteerScreen = props => {
  ...
  const isAvailable = (data) => {
    ...
    const request = fetch('http://10.0.2.2:2018/exam');

    return Promise
      .race([timeout, request])
      .then(response => {
        alert('Action is supported in the server :)')
        fetch("http://10.0.2.2:2018/exam", {
          method: "POST",
          cache: 'no-cache',
          headers: {
  ...
  const savePlaceHandler = async () => {
    ...
    const data = {
      name: nameValue, group: statusDriver, details: statusValue, status: statusCapacity, students: statusPassengers, type: statusPaint
    }
    isAvailable(data)
