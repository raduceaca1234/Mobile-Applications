...
const NewVolunteerScreen = props => {
  const owner = useSelector(state => state.volunteers.name);
  ...
  const isAvailable = (data) => {
    ...
    const request = fetch('http://10.0.2.2:2701/document');

    return Promise
      .race([timeout, request])
      .then(response => {
        alert('Action is supported in the server :)')
        fetch("http://10.0.2.2:2701/document", {
          method: "POST",
          cache: 'no-cache',
          headers: {
  ...
  const savePlaceHandler = async () => {
    ...
    const data = {
      name: nameValue, status: statusValue, owner: owner, size: statusPassengers, usage: statusCapacity
    }
    isAvailable(data)
