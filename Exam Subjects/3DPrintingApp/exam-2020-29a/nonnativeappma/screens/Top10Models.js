...
const Top10Models = props => {
    ...
    const data1 = volunteers.sort((a, b) => (a.time > b.time) ? 1 : ((b.time > a.time) ? -1 : 0));
    ...
    const isAvailable = (operationsQueue) => {
        ...
        const request = fetch('http://10.0.2.2:2901/all');
    ...
    return (
        <FlatList
            ...
            renderItem={itemData => (
                <VolunteerItem
                    name={itemData.item.model}
                    status={itemData.item.status}
                    passengers={itemData.item.client}
                    driver={itemData.item.time}
...
Top10Models.navigationOptions = navData => {
    return {
