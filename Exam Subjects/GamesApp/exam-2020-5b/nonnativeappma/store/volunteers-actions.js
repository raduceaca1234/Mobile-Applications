...
import { fetchAllGames, bookGame, fetchVolunteersLocation, fetchPaints, fetchVolunteersMe, fetchVolunteers, deleteVolunteerForm, createVolunteer, updateVolunteerForm } from '../constants/api';
...
// };
export const bookGames = (id, user) => {
    return async dispatch => {
        try {
            const dbResult = await bookGame({id, user});
...
export const loadAllGames = () => {
    return async dispatch => {
        try {
            const dbResult = await fetchAllGames();
            dispatch({ type: SET_ALL_GAMES, volunteers: dbResult });
        } catch (err) {
            throw err;
        }
    };
