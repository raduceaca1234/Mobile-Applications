...
const Top10Vechicles = props => {
    const volunteers = useSelector(state => state.volunteers.games);
    const operationsQueue = useSelector(state => state.volunteers.operationsQueue);
    const data = volunteers.slice(0, 10);
    data.sort((a, b) => (a.popularityScore > b.popularityScore) ? 1 : ((b.popularityScore > a.popularityScore) ? -1 : 0));
    ...
    const isAvailable = (operationsQueue) => {
        ...
        const request = fetch('http://10.0.2.2:2502/allGames');
        ...
        return Promise
            ...
            .then(response => {
                ...
                dispatch(placesActions.loadAllGames());
            })
            .catch(error => {
                console.log(error)
        ...
        <FlatList
            ...
            renderItem={itemData => (
                <VolunteerItem
                    ...
                    driver={itemData.item.user}
                    capacity={itemData.item.popularityScore}
                    onSelect={() => {
