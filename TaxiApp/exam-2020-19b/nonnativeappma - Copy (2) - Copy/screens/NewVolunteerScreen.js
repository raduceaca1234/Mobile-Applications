...
const NewVolunteerScreen = props => {
  ...
  const isAvailable = (data) => {
    ...
    const request = fetch('http://10.0.2.2:1957/cab');

    return Promise
      .race([timeout, request])
      .then(response => {
        alert('Action is supported in the server :)')
        fetch("http://10.0.2.2:1957/cab", {
          method: "POST",
          cache: 'no-cache',
          headers: {
  ...
  const savePlaceHandler = async () => {
    ...
    // );
    const data = {
      name: nameValue, status: statusValue, size: statusPassengers, driver: statusDriver, color: statusPaint, capacity: statusCapacity
    }
    isAvailable(data)
