...
const NewVolunteerScreen = props => {
  ...
  const isAvailable = (data) => {
    ...
    const request = fetch('http://10.0.2.2:1876/plane');

    return Promise
      .race([timeout, request])
      .then(response => {
        alert('Action is supported in the server :)')
        fetch("http://10.0.2.2:1876/plane", {
          method: "POST",
          cache: 'no-cache',
          headers: {
  ...
  const savePlaceHandler = async () => {
    ...
    // );
    const data = {
      name: nameValue, status: statusValue, size: statusPassengers, owner: statusDriver, manufacturer: statusPaint, capacity: statusCapacity
    }
    isAvailable(data)
